...
DEFINE_string(inet_address,		"",	"local host");
DEFINE_string(server_address,		"", "server ip address");
DEFINE_int32(port,			8910,		"port to listen");
DEFINE_int32(keypoint_port,			8912,		"port to send the pose data");
DEFINE_bool(read_keypoint_json,		false,		"read the keypoint information");
DEFINE_bool(read_keypoint_json_socket,  true,		"read the keypoint information and send back by socket. Not the same port as the image transport!");
...
int openPoseDemo()
{
    ...
    const auto producerSharedPtr = op::flagsToProducer2(FLAGS_image_dir, FLAGS_video, FLAGS_ip_camera, FLAGS_camera,
                                                       FLAGS_camera_resolution, FLAGS_camera_fps, FLAGS_inet_address, FLAGS_port);
    //printf("blocked removed\n");
    ...
    const op::WrapperReader wrapperReader{FLAGS_inet_address, FLAGS_keypoint_port, FLAGS_read_keypoint_json, FLAGS_read_keypoint_json_socket};
    // Configure wrapper
    opWrapper.configure(wrapperStructPose, wrapperStructFace, wrapperStructHand, wrapperStructInput, wrapperStructOutput);
    opWrapper.configure(wrapperReader);
    // std::cout<<"Wrapper reader"<<std::endl;
