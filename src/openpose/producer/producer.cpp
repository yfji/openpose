    ...
    cv::Mat Producer::getFrame()
    {
        try
        {
            ...
            if (isOpened())
            {
                ...
                // sources (e.g. a video)
		if(mType!=ProducerType::Socket && mType!=ProducerType::Database){
		...
		// Check if video capture did finish and close/restart it
			...
			frame=getRawFrame();
			if(frame.empty()){
				// std::cout<<"Empty frame received in producer: "<<mNumberEmptyFrames<<std::endl;
				mNumberEmptyFrames++;
			}
			else	
				mNumberEmptyFrames=0;
			if(mNumberEmptyFrames==10){
				// std::cout<<"No valid frame, empty loop..."<<std::endl;
				mNumberEmptyFrames=0;
			}
		}
    ...
    void Producer::checkFrameIntegrity(cv::Mat& frame)
    {
        try
        {
            ...
            else
            {
                ...
                if (mType != ProducerType::ImageDirectory && mType!=ProducerType::Socket
    ...
    void Producer::ifEndedResetOrRelease()
    {
        try
        {
            if (isOpened())
            {
                ...
                if (mNumberEmptyFrames > 10
                    || (mType != ProducerType::IPCamera && mType != ProducerType::Webcam && mType!=ProducerType::Socket && mType!=ProducerType::Database
