...
namespace op
{
    ...
    __global__ void renderPoseBody25(float* targetPtr, const int targetWidth, const int targetHeight, const float* const posePtr,
                                    const int numberPeople, const float threshold, const bool googlyEyes, const bool blendOriginalFrame,
                                    const float alphaColorToAdd)
    {
        const auto x = (blockIdx.x * blockDim.x) + threadIdx.x;
        ...
        // Render key points
        renderKeypoints(targetPtr, sharedMaxs, sharedMins, sharedScaleF,
                        globalIdx, x, y, targetWidth, targetHeight, posePtr, BODY_25_PAIRS_GPU, numberPeople,
                        POSE_BODY_25_NUMBER_PARTS, numberPartPairs, BODY_25_COLORS, numberColors,
        ...
        try
        {
            if (numberPeople > 0 || !blendOriginalFrame)
            {
                ...
                if (poseModel == PoseModel::COCO_18)
                    ...
                    renderPoseBody25<<<threadsPerBlock, numBlocks>>>(framePtr, frameSize.x, frameSize.y, posePtr, numberPeople,
                                                                     renderThreshold, googlyEyes, blendOriginalFrame, alphaBlending);
