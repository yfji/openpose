...
namespace op
{
                                         ...
                                         const std::vector<double>& scaleRatios)
    {
        try
        {
                ...
                #endif
                const auto& top_blob=upImpl->spHeatMapsBlob.get();
                auto shape=top_blob->shape();
                int h=shape[2];
                int w=shape[3];
                int h_offset=h*w;
                int k_size=13;
                float* heatmapsPtr=top_blob->mutable_cpu_data();
                cv::Mat heatmapImage(h,w,CV_32F,heatmapsPtr);
                #ifdef USE_CUDA
                    cv::cuda::GpuMat gpu_image(heatmapImage);
                    cv::cuda::GpuMat gpu_output(gpu_image);
                    gpu_image.upload(heatmapImage);
                    cv::Ptr<cv::cuda::Filter> gauss = cv::cuda::createGaussianFilter(CV_32F, CV_32F, cv::Size(k_size,k_size),3.,3., cv::BORDER_DEFAULT,-1);
                    gauss->apply(gpu_image, gpu_output);
                    gpu_output.download(heatmapImage);
                #else
                    cv::GaussianBlur(heatmapImage, heatmapImage, cv::Size(k_size, k_size), 3.,3.);
                #endif
