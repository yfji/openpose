...
ProducerType flagsToProducerType2(const std::string& imageDirectory, const std::string& videoPath,
                                 const std::string& ipCameraPath, const int webcamIndex,
			   											   const std::string &ip){
	try
    {
        ...
        if (int(!imageDirectory.empty()) + int(!videoPath.empty()) + int(!ipCameraPath.empty()) > 1)
            ...
            error("Selected simultaneously IP camera and webcam. Please, select only one.",
                  __LINE__, __FUNCTION__, __FILE__);
				if(!ip.empty()){
						if(!ipCameraPath.empty() || !videoPath.empty() || !imageDirectory.empty())
								error("Selected simultaneously socket and others. Please, select only one.",
																  __LINE__, __FUNCTION__, __FILE__);
				}
				if (!ip.empty())
						return ProducerType::Socket;
...
std::shared_ptr<Producer> flagsToProducer2(const std::string& imageDirectory, const std::string& videoPath,
                                                     const std::string& ipCameraPath, const int webcamIndex,
                                                     const std::string& webcamResolution,
                                                     const double webcamFps, const std::string& ip, const int port){

		try
		{
        log("", Priority::Low, __LINE__, __FUNCTION__, __FILE__);
        const auto type = flagsToProducerType2(imageDirectory, videoPath, ipCameraPath, webcamIndex, ip);
				if (type==ProducerType::Socket)
						return std::make_shared<ImageFetcherSocket>(ip, port);
        else if (type == ProducerType::ImageDirectory)
