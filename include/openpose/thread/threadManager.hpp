...
namespace op
{
    ...
    public:
        ...
        inline std::shared_ptr<std::atomic<bool>> getIsBlockingSharedPtr()
        {
            return spIsBlocking;
        }
...
namespace op
{
    ...
    void ThreadManager<TDatums, TWorker, TQueue>::exec()
    {
        try
        {
            ...
            if (!mThreads.empty())
            {
                log("", Priority::Low, __LINE__, __FUNCTION__, __FILE__);
                // Start threads
                mThreads.at(0)->startInThread(spIsBlocking, true);
                for (auto i = 1u; i < mThreads.size() - 1; i++)
                    mThreads.at(i)->startInThread(spIsBlocking, false);
                (*mThreads.rbegin())->exec(spIsRunning, spIsBlocking, true);
    ...
    void ThreadManager<TDatums, TWorker, TQueue>::start()
    {
        try
        {
            ...
            for (auto& thread : mThreads)
                thread->startInThread(spIsBlocking);
            log("", Priority::Low, __LINE__, __FUNCTION__, __FILE__);
        }
        catch (const std::exception& e)
        {
    ...
    void ThreadManager<TDatums, TWorker, TQueue>::multisetToThreads()
    {
		...
		std::cout<<"Thread number: "<<mThreads.size()<<"\nthreadWorker number: "<<mThreadWorkerQueues.size()<<"\nqueue size: "<<mTQueues.size()<<std::endl;
                // Data
    		    ...
    		    std::cout<<"thread id: "<<std::get<0>(threadWorkerQueue)<<", queueIn: "<<std::get<2>(threadWorkerQueue)<<", queueOut: "<<std::get<3>(threadWorkerQueue)<<std::endl;
            ...
            std::cout<<"Model thread queue size: "<<mTQueues[0]->getMaxSize()<<", "<<mTQueues[1]->getMaxSize()<<std::endl;
        }
        catch (const std::exception& e)
        {
            error(e.what(), __LINE__, __FUNCTION__, __FILE__);
