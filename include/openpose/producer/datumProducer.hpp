...
namespace op
{
    ...
    std::pair<bool, std::shared_ptr<TDatumsNoPtr>> DatumProducer<TDatumsNoPtr>::checkIfRunningAndGetDatum()
    {
        try
        {
            ...
            const bool datumProducerRunning = true; // spProducer->isOpened();
            ...
            if (datumProducerRunning)
            {
                ...
                // Get cv::Mat
		//printf("call from datum producer\n");
                ...
                if (datum.cvInputData.channels() != 3)
                {
					...
					if(datum.cvInputData.empty()){
						;//std::cout<<"Empty datum received in datumProducer"<<std::endl;
					}
                    else if (datum.cvInputData.channels() == 1)
                    {
            ...
            */
            ...
            if(!datumProducerRunning)
            	datums=nullptr;
            else if(datum.cvInputData.rows==1 and datum.cvInputData.cols==1)
            	datums=nullptr;
