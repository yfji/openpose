...
namespace op
{
	...
	void configure(const WrapperReader& wrapperReader=WrapperReader{});
...
namespace op
{
    ...
    template<typename TDatums, typename TWorker, typename TQueue>
    void Wrapper<TDatums, TWorker, TQueue>::configure(const WrapperReader& wrapperReader){
try
        {
		typedef std::shared_ptr<TDatums> TDatumsPtr;
		if(wrapperReader.inetAddr.length()>0 and wrapperReader.readKeypointJsonSocket){
			const auto keypointJsonReader = std::make_shared<KeypointJsonReader>();
			const auto spWKeypointJsonReaderSocket=std::make_shared<WKeypointJsonReaderSocket<TDatumsPtr>>(keypointJsonReader, wrapperReader.inetAddr, wrapperReader.port);
			spWKeypointJsonReaderSocket->setBlockingSharedPtr(mThreadManager.getIsRunningSharedPtr());
			mOutputWs.emplace_back(spWKeypointJsonReaderSocket);
		}
		else if (wrapperReader.readKeypointJson)
			    {
				//printf("read keypoint json information\n");
				const auto keypointJsonReader = std::make_shared<KeypointJsonReader>();
				mOutputWs.emplace_back(std::make_shared<WKeypointJsonReader<TDatumsPtr>>(keypointJsonReader));
			    }
		}
        catch (const std::exception& e)
        {
        ...
        try
        {
                ...
                && mThreadManagerMode != ThreadManagerMode::AsynchronousOut)
            {
                ...
                const auto savingSomething = (
                    1 || !wrapperStructOutput.writeImages.empty() || !wrapperStructOutput.writeVideo.empty()
            ...
            if (wrapperStructInput.producerSharedPtr != nullptr)
            {
            	wrapperStructInput.producerSharedPtr->setBlockingSharedPtr(mThreadManager.getIsBlockingSharedPtr());
