...
try
		...
		if(wrapperReader.serverAddr.length()>0 and wrapperReader.readKeypointJsonSocket){
			const auto keypointJsonReader = std::make_shared<KeypointJsonReader>();
			mOutputWs.emplace_back(std::make_shared<WKeypointJsonReaderSocket<TDatumsPtr>>(keypointJsonReader, wrapperReader.serverAddr, wrapperReader.port));
		}
